...
describe('CollectionService - correction and reversal', () => {
  ...
  /**
   * Receipt numbers were once generated by counting the day's existing rows, so
   * deleting one made the next collection reuse its number - and this delete had
   * to refuse whenever a later receipt existed. `generateReceiptNumber` now draws
   * from SequenceService, a monotonic per-day counter that never reissues, so the
   * refusal was removed along with the bug it was compensating for. A later
   * same-day receipt is no longer a reason to block the delete.
   */
  it('deletes even when later receipts exist that day, because numbers are never reused', async () => {
    laterReceiptCount = 2;
    await expect(service.remove(COLLECTION_ID)).resolves.toMatchObject({ deleted: true });
    expect(collections[COLLECTION_ID]).toBeUndefined();
  });
