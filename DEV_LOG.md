...
**Other developer needs to know:** All test delivery tasks and test riders in the DB are cleared. All delivery configuration (Delivery Zones, Delivery Settings, Earning Rules, Failure Reasons, Warehouses, Products, Super Admin account) are fully preserved. You can re-run `npm run delivery:clean` in `svv-balaji-backend` anytime to reset delivery test data.
**Next:** Manual end-to-end testing of delivery app flow.
...
**Touches WS1.5 (sales) and the schema — contract changes below.**

**Schema** (migration `20260926061736_credit_receivables`, additive): enums `CreditPeriodStart` (DISPATCH|ORDER_DATE) and `ReceiptMethod`; `orders.amountPaid` (default 0); `checkout_settings.creditPeriodStart` (default DISPATCH); tables `credit_receipts` (RCPT-YYYYMMDD-NNN, void instead of delete) and `credit_receipt_allocations`. **Deploy: `prisma migrate deploy && prisma generate`, restart API.**

**New module `src/receivables`** (`receivables.logic.ts` = pure due-date/ageing/allocation/statement maths, 18 unit tests):
- A *credit bill* = order with `paymentTerms != PREPAID`, not cancelled, `paymentMode` null (staff) or CREDIT. Due date = credit start + Net N; start is the Super Admin setting (Checkout & Delivery → "5. B2B credit"). Undispatched bills have no due date under DISPATCH.
- `GET /receivables` (list, ageing, `overdueOnly`, branch-scoped), `GET /receivables/customers/:id` (position, ageing, open bills, receipts, statement with `from`/`to` + opening balance), `POST /receivables/customers/:id/receipts` (oldest-due-first allocation under row locks; refuses > outstanding and future dates), `POST /receivables/receipts/:id/void` (reason required; reverts orders). `GET /storefront/credit` = the retailer's own account (staff names stripped). Permissions `receivables.view` (BM, ST) and `receivables.record` (BM) — backfilled onto configured roles by A-14's mechanism.
- Orders marked PAID/REFUNDED by hand before this existed count as settled and appear on the statement as a matching credit.

**Changed behaviour (contract):**
- `PATCH /orders/:id/payment-status` now **refuses** credit bills (use Receivables). `PATCH /orders/:id/cancel` refuses an order with receipts applied (void them first).
- Credit exposure (order-placement limit check, `GET /customers/:id/credit`, `/storefront/auth/me` `creditUsed`) = total − amountPaid on unpaid orders. `/me` counted only `paymentMode=CREDIT` before; now matches the limit check.
- `GET /customers/:id/credit` now also returns `outstanding` + `customerId` — **bug fix:** the admin `CustomerCredit` type read `outstanding`, which was never sent, so the retailer page's Outstanding tile always showed ₹0.
- `/storefront/auth/me` adds `paymentTerms` for retailers. `PATCH /checkout-settings` accepts `creditPeriodStart`.

**Admin:** new screen **CRM & Accounts → Receivables & Credit** (`/receivables`); the retailer page's Credit & Tax tab (`/b2b-customers/:id?tab=credit`) now has position, ageing, open bills with due/overdue, statement (date range), payments received, Record payment (with allocation preview) and Void.
**Customer app:** `/wallet` (retailer) rebuilt on `/storefront/credit`: available credit, overdue alert / next due, bills with due dates, statement, payments. Removed every unconfirmed promise ("up to 20% margin", "₹50,000 credit", "15-day credit", "next-day dispatch") and every fake fallback (`creditLimit || 50000`, `creditUsed || 14500`, GSTIN `36AABCU9603R1ZM`, hardcoded "Net 15 Days") — real values come from `hooks/useRetailerCredit.ts`; guests see "credit terms once approved".
**Still claimed and not confirmed** (left as-is, flag to client): "GST invoices" (OrdersPage perks, PDP row — no invoice generation until A-11), "Same-Day Delivery" / "Same-Day Mill Dispatch" (Home pillars, DesktopHeader).

**Verified:** 573/573 backend unit tests (also fixed the 7 long-failing `customers.service.spec.ts` tests — mock lacked `customerAccount`); new `e2e-receivables-flow.py` 36/36 against the live API (added to `e2e-all.sh`); `e2e-reconcile.mjs` 29/29 (2 new receivables invariants); `tsc` + production builds for backend, admin, customer, field; real-browser screenshots of admin list, credit tab, record-payment modal, retailer wallet and home. Demo data removed.
**Local dev note:** `prisma generate` hit EPERM because the running API holds the engine DLL; the old DLL was renamed to `node_modules/.prisma/client/query_engine-windows.dll.node.locked-*` — delete it after restarting the API.

## 2026-09-26 (latest+1) — Raunak (agent session) — Quick Delivery phase 1: zones + checkout eligibility (backend)

**WS3.4 (delivery).** Also touches checkout (Raunak's) and adds a guard-free call into it from a new module. Quick commerce (15-20 min zones) is not in the FRD as far as found — sits under A-10 (B2C scope, unconfirmed); flagged to the user.

**Review findings that shape the design:** riders were only `Order.riderName/riderPhone` strings (no entity/login); outlets already have lat/lng + radius and checkout already routes LOCAL vs SHIPROCKET (`FulfillmentRouterService`); real-time is `OrderEventsService` -> `/admin` socket (staff only), customer tracking polls; **there is no COD collection or settlement logic anywhere** (a delivered COD order stays PENDING), and admin Outlets / Franchises / Earnings-Finance screens are mock.

**Schema** (migration `20260926065124_delivery_zones_quick`, additive): `DeliveryZone` (boundary polygon JSON, pincodes, maxRadiusKm, targetMin/MaxMinutes — no code defaults, operatingHours + timezone, productScope ALL_PRODUCTS|SELECTED_CATEGORIES, fallback STANDARD|COURIER, quickFee/quickFreeAbove, priority, serving warehouse); `Order.deliverySpeed` (STANDARD|QUICK) + `deliveryZoneId`.
**New `src/delivery`** (`DeliveryModule`): `zones/zone.logic.ts` pure geometry/hours/eligibility (16 unit tests); `ZonesService` CRUD + `quickDecision()`; routes `GET/POST/PATCH/DELETE /delivery-zones`, `POST /delivery-zones/test` (which zone an address lands in and why). Permissions `deliveryZones.view` (BM) / `deliveryZones.manage` (Super Admin only).
**Checkout contract (additive):** `POST /storefront/checkout/{quote,sessions}` accept `deliverySpeed: STANDARD|QUICK` (default STANDARD; QUICK when not possible -> 409 `QUICK_UNAVAILABLE`). Quote gains `fulfillment.speed/zoneId/zoneName` and `deliveryOptions { standard, quick|null }` — `quick` null = do not show the option. Quick = LOCAL from the zone's outlet with the zone's ETA/fee; stock at that outlet always required. Zone fallback COURIER forces courier for the standard option. `FulfillmentRouterService.resolve` takes `courierOnly`.
**Bug fix (checkout, security-relevant):** `recordPaymentFailure` wrote the *unverified* client-supplied `gatewayPaymentId` into the unique column, so a forged confirm with someone's real payment id blocked their genuine confirm (500). Now kept only in `failureReason`. `e2e-checkout-flow.py` payment ids stamped per run (it could not pass twice on one DB).
**Verified:** 589/589 unit tests; `e2e-quick-delivery-flow.py` 26/26 (new; uses non-default 12-18 min / ₹25 so nothing hardcoded passes); `e2e-checkout-flow.py` 102/102; receivables 36/36; reconcile 29/29 — all against a fresh build on :3100 because **the :3000 dev server went down during the session (restart it)**.
**Next:** riders (self sign-up + approval), delivery tasks + auto-offer/accept, rider app API, COD collection + rider cash ledger, failed-delivery reasons, configurable earning rules, `/rider` socket; then admin zone map (OpenStreetMap/Leaflet), customer Quick option, rider app UI.

## 2026-09-26 (latest+2) — Raunak (agent session) — Quick Delivery phase 2: riders, delivery tasks, COD, rider pay (backend)

**WS3.4.** Touches `checkout/fulfillment.service.ts` (assign-rider guard) and reads/writes `orders` rider fields; order statuses unchanged.

**Schema** (migration `20260926071359_riders_delivery_tasks`, 13 new tables, no existing table altered beyond FKs): `Rider` (+ `RiderSession`, `RiderOtp`), `DeliveryTask` (+ events, offers), `CodCollection`, `RiderCashEntry`, `DeliveryFailureReason`, `DeliverySettings`, `RiderEarningRule`, `RiderEarning`, `RiderNotification`. **New env (added to local .env and .env.example): `RIDER_JWT_ACCESS_SECRET`, `RIDER_JWT_REFRESH_SECRET` (32+ chars, must differ from staff/customer), `RIDER_JWT_*_EXPIRES_IN`.** Deploy: migrate deploy + generate + env + restart.
**Module layout:** zones split into `DeliveryZonesModule` (checkout imports it); `DeliveryModule` imports Checkout/Sales/Realtime (never the reverse). `CheckoutModule` now exports `FulfillmentService`.
**Riders:** self sign-up -> phone OTP (storefront OTP mode) -> PENDING_APPROVAL (may sign in, cannot go online) -> staff approve + home outlet -> ACTIVE (code RDR-000001). Password login by phone or email, refresh rotation with reuse detection (revokes session), forgot/reset password by OTP, suspend revokes sessions immediately.
**Dispatch** (`dispatch.service.ts`): packed LOCAL order -> `DeliveryTask` (via `OrderEventsService` + 5 s sweep; idempotent under row lock) -> auto-offer one rider at a time (ACTIVE+ONLINE at the pickup outlet, under task/cash limits, nearest by fresh location else longest idle; timeout/rounds from `DeliverySettings`) -> reject/expire -> next -> none left = `needsManualAssignment` for staff. Accept sets `Order.riderName/Phone` + `RIDER_ASSIGNED` event. Order cancelled -> task cancelled + cancellation pay. Rider coming online retries waiting tasks.
**Task flow** (`task-flow.service.ts`): arrived-pickup (geofence-verified) -> picked-up (**order -> DISPATCHED via SalesService.advance; the stock movement is stamped with the staff member who packed/scanned, fallback assigning staff, then first Super Admin, because StockMovement.performedById must be a User**) -> start -> arrived (geofence) -> COD (exact amount; CASH -> rider cash ledger; marks order PAID + COD PaymentTransaction PAID) -> deliver = existing `FulfillmentService.verifyOtp` (refused until COD collected when `requireCodBeforeDelivery`). Fail with a configured reason (note/photo/arrival requirements) -> order stays DISPATCHED (never auto-cancelled) -> returned-to-store -> staff re-attempt (new task, attempt+1) or AUTO_REATTEMPT reasons. Rider may release before pickup.
**Legacy `POST /orders/:id/assign-rider`** still works and withdraws the board task while no app rider has accepted (race-safe); refused once an app rider holds the order.
**Rider pay** (`earning.logic.ts`, 15 unit tests): rules BASE_PER_DELIVERY, DISTANCE_SLAB, PEAK_HOUR, ZONE_INCENTIVE, DAILY_TARGET, WEEKLY_TARGET, WAITING_TIME (verified arrivals only), OUTCOME_COMPENSATION (cancel stages / failure categories, FIXED or % of delivery). Zone rule replaces the all-zones rule of the same kind; incentives add up; failed/cancelled never earn delivery amounts. Idempotent credit (unique dedupeKey). Default failed-delivery reasons are seeded once (editable).
**API:** `/rider/*` (auth, me, availability, location, dashboard, offers accept/reject, tasks + steps, cod, deliver, fail, returned, release, uploads, failure-reasons, earnings, cash, notifications), `/riders/*` (staff: list/live/approve/reject/suspend/reactivate/update/cash/deposits/earnings/adjustments), `/delivery/*` (tasks board, assign/unassign/redispatch/reattempt, settings, failure-reasons, earning-rules). Socket namespace `/rider` (rider token; own room only; `offer:new`, `offer:closed`, `task:updated`, `notification`; `location` heartbeat). Permissions `riders.view/manage`, `riderCash.record`, `deliveryTasks.view/manage`, `deliverySettings.manage`. Upload folders `delivery-proofs`, `rider-documents`.
**Verified:** 604/604 unit tests; new `e2e-rider-flow.py` 61/61; checkout 102/102; quick 26/26; receivables 36/36; reconcile 32/32 (3 new: COD<->paid<->cash ledger, rider cash never negative, delivered task <-> delivered OTP-verified order + one live task per order). New suites added to `e2e-all.sh`.
**Not built yet:** customer-side real-time (tracking still polls), web push to riders, rider live location on the customer's map, final "undelivered" resolution of a DISPATCHED order whose goods came back (needs a sales transition + stock re-inward). Next: admin screens (zones map, riders, delivery board, pay rules), customer Quick option, rider app UI.

## 2026-09-26 (latest+3) — Raunak (agent session) — Rider app (`svv-balaji-rider`, new) + two backend fixes

**New app `svv-balaji-rider`** (Vite + React + TS strict, port 5176, proxies `/api` and `/socket.io` to the API; installable PWA, shell-only service worker, never caches API). Standalone (no `@shared`): own API client with rider tokens (access in memory, refresh in `svv.rider.refreshToken`, single-flight refresh), `/rider` socket (offers/tasks refetch on events, 30 s location heartbeat while online), Leaflet/OpenStreetMap maps. Screens follow the supplied design (onboarding, Sign In, Create Account, Verification with keypad + resend timer, Select Location, Dashboard with 4 stat tiles + New Orders Accept/Reject, order-detail pop-up Confirm/Reject, orange-pill bottom nav). Adapted from the design: no Facebook/Google login; sign-up asks mobile/city/vehicle instead of Country; brand is SVV. **Built without a design (user to supply):** online/offline switch, request countdown, In-progress list, task screen (map, stepper, navigate/call, one primary action per step), Collect COD, Delivery OTP, Can't deliver, Deliveries, History, Earnings, Cash in hand, Notifications, Profile, side menu, pending-approval screen (licence upload).
**Backend fixes found while screenshotting:** (1) **Privacy** — tasks copied `Order.deliveryAddress`, which ends with the customer's phone, so riders saw the number on a request before accepting. Now built from address fields; `stripPhone()` also applied to offers (older rows). e2e now asserts the digits never appear. (2) **Refresh rotation** logged riders out when a refresh response was lost (retry with the old token = treated as theft). `RiderSession.previousRefreshHash` + `rotatedAt` (migration `20260926075105_rider_refresh_grace`): the just-replaced token is accepted for 2 min; older tokens still revoke the session.
**Verified:** rider app `tsc` strict + `vite build`; headless-Chrome screenshots at 390 px against a seeded demo (no page errors); `e2e-rider-flow.py` 63/63, quick 27/27, 604 unit tests, reconcile 32/32. Demo data removed.

## 2026-09-26 (latest+4) — Raunak (agent session) — Quick Delivery: admin screens + customer Quick option

**Admin** (new menu section **Quick Delivery**, Customer & Retail zone; `leaflet` added to admin deps): **Delivery Board** (`/delivery-board`: every task, needs-a-rider alert/filter, online riders, task drawer with timeline + offers + COD + pay, assign / take back / offer again / re-attempt), **Riders** (`/riders`: status tabs with approval count, approve with home outlet + task limit, reject/suspend with reason, reactivate, drawer with licence photo, cash ledger + record deposit, earnings), **Delivery Zones** (`/delivery-zones`: list with Quick toggle, check-an-address tool, drawer with OpenStreetMap boundary drawing — click to add, drag to move, outlet + radius + other zones shown — pincodes, radius cap, promised minutes, fee/free-above, hours editor incl. overnight, product scope, fallback, priority), **Delivery Settings & Rider Pay** (`/delivery-settings`: pay-rule editors per kind, failed-delivery reasons with requirement toggles and follow-up, dispatch & COD settings). Shared: `shared/api/delivery.ts`, `shared/hooks/useDelivery.ts`, 8 new keys in `shared/auth/permissions.ts`.
**Customer checkout:** `deliverySpeed` sent with quote/session; Delivery card shows Quick (pre-selected when available, greyed with the reason when the zone cannot promise it, hidden outside zones) and Standard with their ETA and fee; a 409 `QUICK_UNAVAILABLE` falls back to Standard with a notice.
**Verified:** admin + customer `tsc`; customer `vite build`; headless-Chrome screenshots of all four admin screens, zone editor map, task drawer, and a 390 px checkout showing "Quick Delivery · In 15-20 min · FREE" vs "Standard · ₹30 · 39-54 min" (no page errors). Demo data removed; reconcile 32/32.
**Not mine, flagged:** untracked `svv-balaji-admin/src/pages/home-sections/` fails `tsc` (`<Can I=...>` / render-prop usage) - someone's work in progress; admin `vite build` is unaffected but `tsc` reports it.

## 2026-09-26 (latest+5) — Raunak (agent session) — Ownership corrected

Raunak owns every workstream (backend, admin, all apps incl. rider/delivery, integrations, testing, deployment); nothing is assigned to Ujjawal. `CLAUDE.md` "Who owns what", `PROJECT_STATE.md` (next-steps heading, A-12/A-13 owners) and today's entries updated accordingly. Historical entries written by Ujjawal are left as the record they are.

## 2026-09-26 (latest+6) — Raunak (agent session) — Rider app: OTP could not be typed

The Verification (and Delivery OTP) screens only reacted to taps on the on-screen keypad, so on a laptop the code could not be entered at all (keyboard / paste did nothing). New `useDigitEntry` in `svv-balaji-rider/src/ui/kit.tsx`: digits, Backspace and paste work alongside the keypad. While OTP is in mock mode the Verification screen shows "Test mode: use 123456" with a Fill button (was a 3-second toast). Server mock code unchanged: `CUSTOMER_OTP_MODE=mock`, `CUSTOMER_OTP_MOCK_CODE=123456` (riders share the storefront OTP setting). Verified in the browser: sign up -> type 123456 -> verified and signed in.

## 2026-09-26 (latest+7) — Raunak (agent session) — Rider app: phone keyboard for OTP, paste works

On-screen keypad removed (Verification and Delivery OTP screens); `useDigitEntry` replaced by `OtpInput` in `svv-balaji-rider/src/ui/kit.tsx`: the design's code boxes sit over one real transparent input (`inputMode="numeric"`, `autocomplete="one-time-code"`, 16 px so iOS does not zoom), so tapping opens the phone's own number keyboard and Android/iOS can offer the SMS code. Paste works for bare codes and whole messages: `cleanCode()` takes the digit group of the right length ("Valid for 5 min: 123456" -> 123456), otherwise all digits. No `maxLength` (it cut pasted messages before they were read). Delivery OTP box count now follows the order's code: rider task detail returns `otpLength` (API addition, the code itself is never sent). Mock OTP unchanged: 123456, with the "Test mode" Fill button. **Verified:** rider `tsc` + `vite build`; headless Chrome at :5176: typing, then pasting "Valid for 5 min: 123456", verified and signed in. Test riders deleted.

## 2026-09-26 (latest+8) — Raunak (agent session) — Admin: Manage Riders hierarchy

**Sidebar:** Quick Delivery now nests a **Manage Riders** sub-menu: Pending Approval (live count badge), All Riders, Live Riders, Rider Earnings, Cash & Deposits, Pay Rules. `NavItem` gained optional `group` (entries sharing a key render as one sub-menu) and `count`; `AppLayout` builds the nested menu, opens the right sub-menu for the current page, and searches group labels. "Delivery Settings & Rider Pay" is now **Delivery Settings** (dispatch & COD, failed-delivery reasons); pay rules moved to `/riders/pay-rules`.
**Pages** (`svv-balaji-admin/src/pages/delivery/riders/`): `/riders/pending` (application cards with licence photo, approve with outlet / reject), `/riders` (status tabs via `?status=`, outlet filter, stat tiles; row opens the rider), **`/riders/:id`** rider page with tabs via `?tab=` (overview + editable dispatch settings, deliveries, earnings with date range + adjustment, cash ledger + record deposit), `/riders/live` (OpenStreetMap, free / busy / stale markers, outlets), `/riders/earnings` (per-rider pay for a period split by earning type, totals row, adjust), `/riders/cash` (who holds how much, cash-limit flag, deposits in a period). Delivery Board opens a task from `?task=<id>`.
**API additions (contract):** `GET /riders/earnings-report?from&to&warehouseId` (default this week) and `GET /riders/cash-report?from&to&warehouseId` (deposits default last 30 days), both `riders.view`; `GET /riders/:id/earnings` from/to now used by the admin. Shared client: `deliveryApi.rider/earningsReport/cashReport`, hooks `useRider`, `useRiderEarningsReport`, `useRiderCashReport`, `EARNING_TYPE_LABEL`.
**Verified:** backend + admin `tsc` (only the known `home-sections` errors), admin `vite build`, both endpoints against live data (bad range -> 400), headless-Chrome screenshots of every new page at 1440 px, no page errors.
**Not built (needs a decision):** rider payouts / settlement of earnings (marking a week paid) - no model for it yet.

## 2026-09-26 (latest+9) — Raunak (agent session) — Rider app: second batch of supplied designs

**Screens (svv-balaji-rider):** order pop-up now shows the product (photo, name, qty, COD, order no., date) with **Confirm / Cancel**; Cancel or Reject opens the **"Write a Specific Reason to Reject Order"** pop-up (scooter art, send button, quick reasons) and the reason is sent with the rejection. New **Orders** tab (`/orders`, replaces "Deliveries"; `/deliveries` redirects) with **New order / Active / Delivered / Returned** tabs (`?tab=`), design cards (product thumb, address, date & time, "Active Order" / "Delivered" chip, pin button). New **Track Your Order** (`/task/:id/track`): full-screen OpenStreetMap, rider's live position to the store (before pickup) or customer (after), dark contact card with chat + call, then Address (Navigate), Delivery Time (ETA range + due time) and Order#; task screen map links to it. **Password Reset** success screen after a reset. Shared order UI moved to `src/ui/orders.tsx`.
**Adapted from the design:** chat opens **WhatsApp** and call opens the **phone dialer** - there is no in-app chat or in-app calling service, so the design's Chat and Calling screens were not built (needs a decision: masked calling / chat provider). Route line is a straight dashed line (no routing engine); Navigate hands over to Google Maps.
